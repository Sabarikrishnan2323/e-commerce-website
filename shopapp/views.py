...
def add_item(request):
  try:  
     ...
     return Response({"data":serializer.data,"message":"Cartitem is ceated successfully"},status=201)
...
def payment_callback(request):
   ...
   if status=='successful':
      ...
      if response_data['status'] == 'success':
         ...
         and float(response_data['data']['amount']) == float(transaction.amount) and response_data['data']['currency']==transaction.currency):
         ...
         return Response({'message':'Failed to verify transaction with Flutterwave.','subMessage':"We coudn't verify transaction"})
   else:
   ...
   if request.method == 'POST' and request.user.is_authenticated:
      ...
      payment=paypalrestsdk.Payment({
         ...
         "payer":{
            ...
            "cancel_url":f"{BASE_URL}/paymentstatus?paymentStatus=cancel"
         },
         "transactions":[{
            "item_list":{
               "items":[{
